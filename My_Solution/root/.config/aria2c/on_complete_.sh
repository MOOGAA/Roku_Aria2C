...
if [[ $2 -eq 1 ]]; then # single file
	rclone -v --config="~/.config/rclone/rclone.conf" copy "$3" "$REMOTE:$RCLONE_DESTINATION" 2>&1	
elif [[ $2 -gt 1 ]]; then # multiple file
	rclone -v --config="~/.config/rclone/rclone.conf" copy "$topPath" "$REMOTE:$RCLONE_DESTINATION/${relativePath%%/*}"
fi
